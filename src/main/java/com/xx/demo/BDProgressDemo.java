...
public class BDProgressDemo extends Application {
    ...
    public void start(Stage stage) throws Exception {
        ...
        VBox controlCenter = new VBox(10);
        BDTaskControlCenter taskControlCenter = new BDTaskControlCenter();
        BDButton insertTask1 = new BDButton("插入任务", Util.getImageView(20, BDIcon.ADD_DARK));
        insertTask1.setSelectable(false);
        mapping.addEventHandler(insertTask1, ActionEvent.ACTION, _ -> taskControlCenter.pushTask(getSimpleTask(),true,true));
        controlCenter.getChildren().addAll(taskControlCenter,insertTask1);
        controlCenter.setPadding(new Insets(20));

        VBox root = new VBox(20);
        root.getChildren().addAll(new Text("单个任务："), progress,new Text("任务管理中心组件："),controlCenter);
